...
class StateGraph(metaclass=StateGraphMeta):
   ...
   """
    ...
    def go_to_state(self, to_state: State | str, **kwargs) -> bool:
        ...
        while self.current_state != to_state and counter < max_transitions:
            ...
            try:
                ...
                gtl_logger.info(f'Going from state {self.current_state} to {to_state}, calling transition {transition.ui_actions.__name__}')
    ...
    def _validate_state(self, expected_state: State, **kwargs):
        ...
        if not valid:
            gtl_logger.error(f'Expected to be in state {expected_state}, but the validation failed')
            ...
            gtl_logger.error(f'Was in the expected state {expected_state}, but context {kwargs} did not match')
            ...
            gtl_logger.info(f'Validated that current state is the expected state {expected_state}')
    ...
    def recover_state(self, expected_state):
        ...
        :param expected_state: The state that is expected to be the current state.
        """
        gtl_logger.info(f"Recovering state to {expected_state}")
        # Ensure app active
